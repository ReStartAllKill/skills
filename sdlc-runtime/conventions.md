...
폴더당 spec은 하나이고, 검사기는 **그 폴더의 plan이 spec의 Must 수용 기준을 전부 덮어야** 통과시킨다
(규칙 5-5). 레포마다 자기 몫만 덮는 plan은 셋 다 오류가 난다. 그래서 **spec은 코드 레포 단위로 한 벌**
쓴다 — intent는 하나를 공유하고, 레포마다 폴더를 두어 같은 intent 사본과 그 레포의 spec을 담는다.
문서 레포가 상류이면 승인된 intent·spec 사본을 코드 레포의 `spec_dir`로 끌어와 그 옆에 plan을 쓴다.
사본은 커밋한다 — `spec_version`이 거기서 찍힌다. spec을 고칠 일은 상류에서 `/iterate-spec`으로 하고
다시 끌어온다. 사본을 직접 고치면 정본이 둘이 된다.
