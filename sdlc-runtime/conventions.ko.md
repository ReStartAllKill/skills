...
- 슬라이스: 시나리오에 단 우선순위. `Must` 시나리오는 그 하나만 되어도 내보낼 가치가 있다는 뜻이고,
  요구사항이 `시나리오:`로 자기가 실현하는 시나리오를 가리켜 슬라이스가 plan 에 닿는다. 달면 모든
  시나리오에 달고, 안 달면 어디에도 안 단다 — 우선순위 없는 spec 은 슬라이스가 하나다.
...
| `SCN-*` | 시나리오. 우선순위가 붙으면 배포 슬라이스 | spec §시나리오 |
