...
- **v7 adds a sixth artifact, `research`.** A `research.md` lives at `<spec_dir>/research/RSH-YYYY-NNN-<slug>/research.md`, outside artifact sets, and always uses `schema_version: 7` — as an ADR always uses 5. It records what an investigation read: `Question`, `Criteria` (`CRIT-*`), `Sources` (`SRC-*`, each with `at:` and `retrieved: YYYY-MM-DD`), `Options` (`OPT-*`, each with a `basis:` naming a source), a comparison table of criteria by option, `Judgement` (`REC-*`, each with a `basis:` naming an option) and an optional `Open questions` (`RQ-*`).
  - **It has no approval and no tier.** Statuses are `draft | in_review | reviewed`; `reviewed` means a person read it, not that anyone accepted it, so the approval guard treats no research transition as an approval and `approved_by` and `tier` are errors in its frontmatter. Evidence is not a decision — what was decided lives in the ADR or intent that cites this document, and that is where the approval belongs.
  - **It sits outside artifact sets because it precedes them and outlives one.** An investigation runs before there is an intent to attach it to, and one investigation is usually cited by several documents. Hanging it inside one set would make the other citations point into someone else's change.
  - **Other documents cite it inline, as `RSH-2026-001` or `RSH-2026-001/SRC-002`** (also `/OPT-NNN`, `/REC-NNN`). The checker resolves every such reference against `<spec_dir>/research/` and reports a dangling one as an error; where no profile is found it says the citations were not resolved rather than passing quietly. There is no frontmatter key for the pin: a citation belongs in the sentence it supports, where “what makes you say that” is read.
