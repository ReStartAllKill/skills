...
# ${why}
#
# **This file contains no hook logic.** It is a pointer to <sdlc_runtime>/tools/${tool},
# so a hook fix does not require updates across every repository. A vendored runtime takes
# precedence: vendoring exists to keep the team and CI on the same checker, and letting hooks
# use only the global copy would break that guarantee.
#
# Generated by install-hook.mjs. Update the source instead of editing this file.
set -uo pipefail

ROOT="\${CLAUDE_PROJECT_DIR:-}"
[ -n "$ROOT" ] || ROOT="$(git -C "$(dirname "$0")" rev-parse --show-toplevel 2>/dev/null)" || exit 0

# Runtime discovery order is policy: the profile-selected copy takes precedence, followed by
# the conventional vendor path and installed plugin copies. Do not source sdlc-lib.sh here:
# that file lives inside the runtime and cannot be loaded before the runtime is found. Read the
# profile directly with sed for the same reason.
#
# Without the profile lookup, hooks could use the installed copy while skills and CI use the
# selected copy. The same document would then produce different results, and runtime development
# repositories would always run their hooks against an older harness.
PROFILE_RT=""
...
const SHIMS = [
  {
    ...
    why: 'PostToolUse(Edit|Write) — checks traceability across documents after an artifact is saved.',
  },
  {
    file: 'sdlc-approval.sh', tool: 'guard-approval.sh', event: 'PreToolUse', matchers: ['Edit|Write', 'Bash'], timeout: 15,
    why: 'PreToolUse(Edit|Write|Bash) — blocks self-approval and unapproved changes to approved documents.',
  },
]
