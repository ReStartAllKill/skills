...
5. `assets/<lang>/spec-template.md`로 작성한다. FR/NFR의 `근거:`에 OUT/CON을 연결하고,
   ...
   시나리오에 `Must`·`Should`·`Could` 를 달면 그것이 **배포 슬라이스** 다 — `Must` 시나리오는 그 하나만
   되어도 내보낼 가치가 있다는 뜻이다. 달았으면 모든 시나리오에 달고, 요구사항이 `시나리오:` 로 자기가
   실현하는 시나리오를 가리키게 한다. 검사기는 어느 요구사항도 가리키지 않는 `Must` 시나리오를 경고한다.
   작은 변경이라 슬라이스가 하나뿐이면 우선순위를 달지 않는다.
