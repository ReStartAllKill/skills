...
이미 열린 선형 PR chain 이나 native stack 에 등록되지 않은 legacy PR 은 아래처럼 bottom 부터 top 순서로
등록한다. `gh stack link` 가 PR base 를 chain 에 맞게 연결하지만 local tracking 은 만들지 않으므로, link
...
`gh stack link` 이 성공해도 base chain 만 맞고 native stack object 가 없을 수 있으므로, REST API 의
...
강제 push 로 우회하지 않는다. extension·version·auth 사전 조건이나 linear chain 조건이 충족되지 않으면
