...
2. 런타임의 `conventions.md`, `references/schema.md`를 읽는다. 작업 수정에는
   `references/tasks.md`를 추가로 읽고, `references/prose.md`는 검사가 낸 문체 경고를
   해석해야 할 때만 읽는다.
...
상위 문서를 커밋한 뒤 하위 문서가 참조하는 버전을 갱신한다. 참조 버전이 v7의 본문 핀(`body:`)
이면 상위 본문을 고치는 순간 핀이 어긋나므로 `node <sdlc_runtime>/tools/pin.mjs <상위 파일>`로
다시 뽑아 넣는다. 상태·승인처럼 frontmatter만 바뀐 편집은 본문 핀을 바꾸지 않으니 갱신할 것이 없다.
...
3. `node <sdlc_runtime>/tools/check-set.mjs <명세 디렉터리>`를 실행한다.
   상위 문서의 참조 버전, Must 요구사항과 작업의 연결, 같은 실행 레벨의 파일 충돌을 확인한다.
4. 오류를 해결한 뒤 공통 절차에 따라 필요한 재승인을 처리한다. 재승인 뒤 다시 검사하지 않는다 —
   마지막 확인은 아래 `plan-progress.mjs`다. 실행 중이던 plan은 재승인 직후
