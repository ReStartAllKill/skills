...
메인 작업 트리는 결과를 병합할 현재 체크아웃이며, plan의 `target_branch`에서 재개한다.
대상 브랜치가 없거나 현재 브랜치가 다르면 사용자가 요청한 작업 범위와 기존 변경을 확인해
브랜치를 준비한다. 다른 사람의 미커밋 변경은 보존한다.

```sh
node <sdlc_runtime>/tools/plan-resume.mjs <명세 디렉터리>
```

이 읽기 전용 도구는 체크박스·귀속 커밋·작업 브랜치·검증 로그를 대조해 다음 행동을 알려준다.
`plan-levels`는 의존 관계만 계산하므로 재개 지점으로 사용하지 않는다.

| action | 이어서 할 일 |
|---|---|
| `implement` | 표시된 레벨의 표시된 작업만 2a부터 구현 |
| `resume_worktree` | 기존 워크트리 변경과 bootstrap 상태를 확인하고 2b부터 재개 |
| `merge` | 이미 있는 작업 커밋을 병합하고 워크트리를 정리한 뒤 다시 조회 |
| `verify_scoped` | 2d의 중간 검증을 실행하고 다시 조회 |
| `verify_full` | 모든 작업 ID로 마지막 전체 검증과 추가 게이트 실행 |
| `record` | 2e에서 미체크 작업만 기록하고 각 레벨 로그와 plan 커밋 |
...
전체 검증 전에는 체크하지 않고, 후속 레벨을 구현한다.

첫 구현 전에 plan을 `in_progress`로 변경해 커밋한다. 진행은
`assets/<대화 언어>/report-templates.md`의 「진행 상태」 형식으로 보고한다.

## 2. 실행 레벨별 구현

`plan-resume`이 지정한 다음 행동과 `plan-levels`의 의존 순서를 따른다. 실행 레벨은 작업 의존 관계로 구분한 단계이며,
...
`--note`에는 무엇이 어떻게 달랐는지 핵심만 한 문장으로 적는다. 원인 분석, 시도한 것, 로그
발췌, 코드 설명은 커밋 메시지와 verify 로그에 이미 있으므로 옮겨 적지 않는다. 한도를 넘으면
도구가 거부하며, 그때는 늘려 쓸 자리를 찾지 말고 줄인다.
