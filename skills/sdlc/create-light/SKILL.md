...
5. `node <sdlc_runtime>/tools/check-set.mjs <폴더>`를 **폴더에 한 번** 실행한다. 검사기와 린터를
   ...
   뒤 결과를 그대로 보고한다.
6. 셋 다 `status: in_review`로 저장하고 요약을 보고한 뒤, intent → spec → plan 순으로 승인 편집을
   시도한다. 편집 하나가 `status: accepted`와 `approved_by`를 함께 쓰고, 가드가 그 편집마다 승인
   다이얼로그를 띄운다 — 다이얼로그 세 번은 사람의 판단 세 번이고 줄이지 않는다.
   `approved_by`의 규칙은 공통 절차와 같다. 하나가 거절되면 거기서 멈춘다 — 아래 문서는
   `in_review`로 남고 아무것도 커밋하지 않는다.
7. 셋 다 승인된 뒤 세 파일을 **한 커밋**에 담는다 — 공통 절차대로 승인 뒤 재검사는 하지 않는다.
