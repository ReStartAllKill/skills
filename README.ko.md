...
변경을 산출물 사슬 — 의도 · 명세 · 계획 · 구현 — 로 지나가게 하고, 그 사슬이 맞물리는지를
**쓰는 그 자리에서** 검사하는 Claude Code 플러그인.

> **산출물 언어는 한국어다.** 글자 한도는 한국어 밀도로 잰 수이고 문체 검사도 한국어 표현
> 목록이다. 구조·추적성 검사는 언어와 무관하다.
>
> 영문으로 써도 사슬은 돌지만 린터가 문서마다 `lang-unsupported` 를 낸다 — 문체 검사 셋이
> 하나도 안 걸리고 길이 한도는 두 배쯤 조이기 때문이다. 그 침묵이 «깨끗하다» 로 읽히지
> 않게 하려고 두는 경고다.
...
| **CI 검사기** | `check-all.mjs` | 레포의 **모든** 사슬을 본다. 훅은 개인 장비에 있어 팀원에게 없을 수 있고, 그래서 훅만으로는 관문이 못 된다. |

게이트는 방금 만진 폴더만 본다. 그래서 상위 문서가 바뀐 뒤 아무도 안 건드린 하위는 조용히
낡아간다 — 이런 사슬에서 가장 흔한 붕괴가 정확히 그 모양이고, CI 검사기는 그것을 잡으려고
있다.
...
| 스킬 | 언제 |
|---|---|
| `/sdlc-init` | 레포에 사슬을 깔 때 |
| `/create-finding` | 장애 · 알림 · 이상 지표 · 스캔 결과를 사슬의 입력으로 만들 때 |
