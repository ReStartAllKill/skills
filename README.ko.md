...
작은 변경을 처음 진행한다면 `/sdlc-init` → `/create-light <변경 내용>` →
`/implement-spec <산출물 폴더>` 순서로 시작한다. 검토할 때는 변경 동작·위험·미결정 사항을
확인한다. 중단한 구현은 다음 명령으로 이어갈 위치를 확인할 수 있다.

```sh
node <sdlc_runtime>/tools/plan-resume.mjs <산출물 폴더>
```

프로필에서 `approval_mode: batch_light`를 선택하면 작은 변경의 세 문서를 한 번의 다이얼로그로
승인한다. 검토한 내용의 해시가 바뀌면 다시 검토해야 한다. `lint_warnings: advisory`는 문체
경고를 표시하되 CI를 차단하지 않고, `error`는 저장 시점과 CI 모두에서 차단한다.
...
| `SCN-*` | 시나리오 — 우선순위가 붙으면 배포 슬라이스. 요구사항이 `시나리오:`로 가리킨다 | spec §시나리오 |
...
쓴다 — intent·spec·plan을 그대로 쓰고 같은 검사기·린터를 돌리고 프로필에 따른 승인 절차를 거쳐, 한 번에
쓰고 한 커밋에 담는다. 그 밖의 변경은 `/create-intent` → `/create-spec` → `/create-plan`으로
가며, 각 단계는 다음 문서를 쓰기 전에 검토에서 멈춘다. `sdlc_version`이 7 이상이어야 한다.

`/implement-spec`은 같은 레벨의 작업을 별도의 Git 워크트리에서 병렬로 실행한다. 중간 레벨은 범위 검증을,
마지막 레벨은 전체 검증을 수행한다.
